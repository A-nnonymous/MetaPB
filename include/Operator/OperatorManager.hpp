...
void trainModel(regressionTask task) {
  ...
  for (auto &[opTag, batchUpBound_MiB] : task) {
    if (batchUpBound_MiB > maxMiB) maxMiB = batchUpBound_MiB;
  }
  ...
  opMap[OperatorTag::REDUCE] = std::move(this->getOperator(OperatorTag::REDUCE));
...
void verifyAll(const std::string &filePath, const size_t batchUpperBound_MiB){
...
inline void execCPU(OperatorTag opTag, const size_t batchSize_MiB, void** memPoolBffrPtrs)const noexcept{
  opMap.at(opTag)->execCPU(batchSize_MiB, memPoolBffrPtrs);
}
inline void execDPU(OperatorTag opTag, const size_t batchSize_MiB)const noexcept{
...
perfStats deducePerfCPU(OperatorTag opTag,
                     size_t batchSize_MiB) {
  return opMap.at(opTag)->deducePerfCPU(batchSize_MiB);
}
perfStats deducePerfDPU(OperatorTag opTag,
                     size_t batchSize_MiB) {
