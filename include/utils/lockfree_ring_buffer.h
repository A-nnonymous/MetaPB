...
static_assert(std::atomic_size_t::is_always_lock_free,
              "atomic_size_t must be always lock free");
...
public:
  lockfree_ring_buffer_t(uint32_t power_of_2_size)
      : m_capacity(next_power_of_2(power_of_2_size)),
        m_power_of_2_mod(m_capacity - 1), buffer(m_capacity, nullptr) {
  ...
  int try_push(void *in) {
    assert(in); // can't store NULLs; we rely on a NULL to indicate a spot in
                // the buffer has not been written yet
    // read low first; this means the buffer will appear larger or equal to its
    // actual size
    ...
    if (!this->buffer[index] && high - low < this->m_capacity &&
  ...
  void *try_pop() {
    // read high first; this means the buffer will appear smaller or equal to
    // its actual size
    const uint64_t high = this->m_high.load(std::memory_order_acquire);
    // load_load_barrier();
  ...
  int unsafe_initial_dynamic_push(void *in) {
    assert(m_low == 0 && "you must not unsafe push once an item has been "
                         "popped from this array");
    if (this->m_high - this->m_low >= this->m_capacity) {
  ...
  // call back function
  void unsafe_for_each(void (*fun)(void *item, void *user_data),
                       void *user_data) {
