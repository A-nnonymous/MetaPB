...
void DPUWisePull(DpuSet *Rank, const size_t idx,
                 vector<vector<uint32_t>> &result, size_t bufferSize) {
...
void DPUWisePush(const vector<vector<uint32_t>> &data, DpuSet *Rank,
                 const size_t idx) {
  auto &dpu = Rank->dpus()[idx];
...
void threadRankPull(vector<rankData> &result, DpuSet *wholeSet, size_t idx) {
  wholeSet->ranks()[idx]->async().copy(result[idx], DPU_MRAM_HEAP_POINTER_NAME,
                                       0);
  // wholeSet->ranks()[idx]->copy(result[idx], DPU_MRAM_HEAP_POINTER_NAME, 0);
}

void summonPush(DpuSet *wholeSet, const vector<rankData> &src,
                size_t usedRank) {
  vector<thread> tv;
  ...
  */
  ...
  for (auto usedRank = 1; usedRank <= 32; usedRank <<= 1) {
    ...
    for (int i = 0; i < rep + warmup; i++) {
      if (i >= warmup)
        ct.tick("RankWisePush_32MiB_" + std::to_string(usedRank) + "Rank");
      officialPush(usedRank, dpuBuffer);
      if (i >= warmup)
        ct.tock("RankWisePush_32MiB_" + std::to_string(usedRank) + "Rank");

      if (i >= warmup)
        ct.tick("RankWisePull_32MiB_" + std::to_string(usedRank) + "Rank");
      officialPull(usedRank, dpuBuffer);
      if (i >= warmup)
        ct.tock("RankWisePull_32MiB_" + std::to_string(usedRank) + "Rank");
