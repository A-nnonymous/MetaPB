...
TaskGraph genSingleOp_w_reduce(const OperatorTag &opTag, const size_t batchSize_MiB) {
  TaskProperties op = {opTag, OperatorType::Undefined,
                           batchSize_MiB, "", ""};
...
TaskGraph genSingleOp_wo_reduce(const OperatorTag &opTag, const size_t batchSize_MiB) {
  TaskProperties op = {opTag, OperatorType::Undefined,
                           batchSize_MiB, "", ""};
  Graph g;
  auto opNode = boost::add_vertex(op, g); 
