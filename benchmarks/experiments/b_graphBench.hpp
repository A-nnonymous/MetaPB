...
public:
  ...
  void writeCSV(const std::string &dumpPath) const noexcept override {
    std::ofstream file(dumpPath + myArgs.at("loadSize_MiB") + "MiB_" +
                       +"wholesome_perf.csv");
    file << "workload_name,scheduler_name,timeConsume_Seconds,timeConsume_"
            "deduce_Seconds,energyConsume_Joules,energyConsume_deduce_Joules,"
            "dataTransfer_MiB\n";
    for (const auto &[expTag, stats] : testResults) {
      const auto &[loadName, schedName] = expTag;
      file << loadName << "," << schedName << ","
  ...
  void exec() override {
    ...
    for (int i = 20; i <= 80; i += 30) {
      graphLoads[("H-" + std::to_string(i))] =
          randomizeHO(i / 100.0f, loadSize_MiB);
      graphLoads[("M-" + std::to_string(i))] =
          randomizeMO(i / 100.0f, loadSize_MiB);
    }
    for (int i = 4; i <= 8; i *= 2) {
    ...
    */
      graphLoads[("M-" + std::to_string(80))] =
          randomizeMO(80.0f, loadSize_MiB);
    ...
    for (const auto &[loadName, loadGraph] : graphLoads) {
      ...
      MetaScheduler metaPF(0.6f, 0.4f, 200, loadGraph, om);
      MetaScheduler metaHY(0.5f, 0.5f, 200, loadGraph, om);
      MetaScheduler metaEF(0.4f, 0.6f, 200, loadGraph, om);
      ...
      for (const auto &[schedName, sched] : scheduleResult) {
        std::cout << "executing " << schedName << "'s schedule on " << loadName
                  << "\n";
        perfStats stat;
        deduceResults[{loadName, schedName}] =
            hcp.execWorkload(loadGraph, sched, execType::MIMIC);
        for (int i = 0; i < WARMUP_REP + REP; i++) {
          ...
          if (i >= WARMUP_REP) {
            ...
            if (i == WARMUP_REP + REP - 1) {
              hcp.outputTimingsToCSV(
                  "/output/MetaPB_Results/All_Graphloads/Timings_" + loadName +
                  "_" + schedName + ".csv");
            }
          }
        }
