...
void ChronoTrigger::dumpAllReport(std::string path) {
  ...
  for (const auto &[tag, metricName] : metricNameMap) {
    ...
    for (const auto &[taskName, report] : task2Report) {
      const auto correctedReport =
          taskName == "__BIAS__" ? report : report - task2Report["__BIAS__"];
      ...
      if (metric.data.index() == 0) {
        ...
        items[0] = {
            taskName, std::to_string(data.rep), std::to_string(data.sum),
            std::to_string(data.mean), std::to_string(data.variance),
            std::to_string(data.stdVar),
            // Normally there's no data that is untouched, but noise purging
            // might introduce them.
            std::to_string((data.upperBound == -__DBL_MAX__) ? 0.0f
                                                             : data.upperBound),
            std::to_string((data.lowerBound == __DBL_MAX__) ? 0.0f
                                                            : data.lowerBound),
            std::to_string(data.upperBias), std::to_string(data.lowerBias)};
      } else [[likely]] { // plural metric (socket-wise)
        const vector<Stats> &datavec = std::get<vector<Stats>>(metric.data);
        items.resize(datavec.size());
        for (auto i = 0; i < items.size(); ++i) {
          items[i].resize(metricTag::size + 1);
          items[i] = {taskName + "_" + std::to_string(i),
                      ...
                      // Normally there's no data that is untouched, but noise
                      // purging might introduce them.
                      std::to_string((datavec[i].upperBound == -__DBL_MAX__)
                                         ? 0.0f
                                         : datavec[i].upperBound),
                      std::to_string((datavec[i].lowerBound == __DBL_MAX__)
                                         ? 0.0f
                                         : datavec[i].lowerBound),
