...
struct Channel {
  void *handle; // storage for channel payload: one single handle. void means
...
class task_list_o {
  ...
  std::atomic_size_t num_tasks; // number of tasks, only gets decremented if
                                // taks has been removed

public:
  std::atomic_flag
      block_flag; // flag used to signal that dependent tasks have completed

  task_list_o(uint32_t capacity_hint = 32) // start with capacity of 32
      : tasks(capacity_hint), num_tasks(0) {}

  ~task_list_o() {
    // If there are any tasks left on the task list, we must destroy them, as we
    // own them.
    void *task;
  ...
  inline size_t get_tasks_count() { return num_tasks; }
  ...
  void tag_all_tasks_with_scheduler(scheduler_impl *p_scheduler) {
    tasks.unsafe_for_each(
        [](void *c, void *p_scheduler) {
          coroutine_handle_t::from_address(c).promise().scheduler =
              static_cast<scheduler_impl *>(p_scheduler);
        },
        p_scheduler);
  }
...
void TaskList::add_task(Task c) {
  assert(p_impl != nullptr &&
         "task list must be valid. Was this task list already used?");
  p_impl->add_task(c);
}
...
public:
  std::vector<Channel *>
      channels; // non-owning - channels are owned by their threads
  std::vector<std::jthread> threads;

  scheduler_impl(int32_t num_worker_threads = 0);

  ~scheduler_impl() {
    // We must unblock any threads which are currently waiting on a flag signal
    // for more work as there is no more work coming, we must artificially
    // signal the flag so that these worker threads can resume to completion.
    for (auto *c : channels) {
      if (c) {
        c->flag.test_and_set(); // Set flag so that if there is a worker blocked
                                // on this flag, it may proceed.
        c->flag.notify_one(); // Notify the worker thread (if any worker thread
                              // is waiting) that the flag has flipped. without
                              // notify, waiters will not be notified that the
                              // flag has flipped.
      }
    }
    // We must wait until all the threads have been joined.
    // Deleting a jthread object implicitly stops (sets the stop_token) and
    // joins.
    threads.clear();
  }

  // Execute all tasks in the task list, then invalidate the task list object
  void wait_for_task_list(TaskList &tasklist_wrapper);
};
...
scheduler_impl::scheduler_impl(int32_t num_worker_threads) {
  ...
  if (num_worker_threads < 0) {
    ...
    num_worker_threads =
        std::jthread::hardware_concurrency() + num_worker_threads;
  }

  assert(num_worker_threads >= 0 &&
         "Inferred number of worker threads must not be negative");

  // std::cout << "Initializing scheduler with " << num_worker_threads << "
    ...
    threads.emplace_back(
        ...
        [](std::stop_token stop_token, Channel *ch) {
          while (!stop_token.stop_requested()) {
            ...
            if (ch->handle) {
              coroutine_handle_t::from_address(ch->handle)
...
void scheduler_impl::wait_for_task_list(TaskList &tasklist_wrapper) {

  if (tasklist_wrapper.p_impl == nullptr) {
    assert(false && "Task list must have been freshly initialised. Has this "
                    "task list been waited for already?");
    return;
  }
  ...
  tasklist_wrapper.p_impl->tag_all_tasks_with_scheduler(this);

  // Distribute work, as long as there is work to distribute
  while (tasklist_wrapper.p_impl->get_tasks_count()) {

    // ----------| Invariant: There are Tasks in this Task List which have not
    // yet completed

    coroutine_handle_t c = (tasklist_wrapper.p_impl)->pop_task();

    if (c == nullptr) {
      // We could not fetch a task from the task list - this means
      // that there are tasks in-progress that we must wait for.

      if (tasklist_wrapper.p_impl->block_flag.test_and_set(std::memory_order::acq_rel)) {
        std::cout << "blocking thread " << std::this_thread::get_id() << " on ["
                  << tasklist_wrapper.p_impl << "]" << std::endl;
        // Wait for the flag to be set - this is the case if any of these
        // happen:
        //    * the scheduler is destroyed
        //    * the last task of the task list has completed, and the task list
        //    is now empty.
        tasklist_wrapper.p_impl->block_flag.wait(true, std::memory_order::acquire);
        std::cout << "resuming thread " << std::this_thread::get_id() << " on ["
                  << tasklist_wrapper.p_impl << "]" << std::endl;
      } else {
        std::cout << "spinning thread " << std::this_thread::get_id() << " on ["
                  << tasklist_wrapper.p_impl << "]" << std::endl;
      }

      continue; // Retry handle getting
    }
    ...
    if (move_task_to_worker_thread(c)) {
      // Pushing consumes the coroutine handle - that is, it becomes owned by
      // the channel who owns it for the worker thread.
      //
      // If we made it in here, the handle was successfully offloaded to a
      // worker thread.
      //
      // The worker thread must now execute the payload, and the task will
      // decrement the counter for the current TaskList upon completion.
      continue;
    }

    // --------| Invariant: All worker threads are busy - or there are no worker
    // threads: we must execute on this thread
    c();
  }

  // Once all tasks have been complete, release task list
  delete tasklist_wrapper.p_impl;    // Free task list impl
  tasklist_wrapper.p_impl = nullptr; // Signal to any future users that this task list has
                        // been used already
}
...
void Scheduler::wait_for_task_list(TaskList &tasklist_wrapper) {
  p_impl->wait_for_task_list(tasklist_wrapper);
}
...
Scheduler::~Scheduler() { delete p_impl; }

// ----------------------------------------------------------------------

void suspend_task::await_suspend(
...
void finalize_task::await_suspend(
    std::coroutine_handle<TaskPromise> h) noexcept {
