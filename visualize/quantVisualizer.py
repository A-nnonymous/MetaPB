...
def x_format_func(value, tick_number):
    ...
    if num_digits <= 3:
        ...
        return '{:.0}'.format(value)
def generate_plots(data_directory, output_directory):
    # Ensure output_directory exists
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)

    # Specify the directory structure
    directories = [os.path.join(data_directory, d) for d in os.listdir(data_directory) if os.path.isdir(os.path.join(data_directory, d))]
...
if len(sys.argv) != 3:
    print("Usage: python this.py <path_to_batch_folder> <output_path>")
    sys.exit(1)

# Example usage:
data_directory = sys.argv[1]
output_directory = sys.argv[2]
generate_plots(data_directory, output_directory)